...
# Jugador principal con movimiento discreto estilo Crossy Road.
# Cada tecla mueve al personaje exactamente una casilla y reproduce un salto visual.
...
func _ready() -> void:
	...
	# Importante: el GLB original tenía animaciones de posición que separaban partes.
	# Por eso no se reproducen automáticamente.
...
func _read_discrete_input() -> Vector3:
	# Solo se acepta una dirección por salto para mantener movimiento lineal.
	# W / Flecha arriba = adelante del mapa.
...
func _start_hop(direction: Vector3) -> void:
	...
	# Si el movimiento no cambia de casilla, no se ejecuta salto.
...
func _update_hop(delta: float) -> void:
	...
	# La raíz se mueve horizontalmente; el modelo visual hace el salto.
...
func _face_direction(direction: Vector3) -> void:
	# En Godot el frente visual suele apuntar a -Z.
